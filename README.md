...
- 资产：资产页分为“添加资产”和“记录交易”两条路径；添加资产只负责建立资产档案和初始持仓，默认展示名称、分类、账户、币种、数量、成本价和开仓日期等必要字段；已有持仓行中的“记录交易”用于加仓、减仓和清仓。账户类型是大类预设并支持自定义，账户名称按账户类型提供常见选项，也可新增自定义名称；支持输入代码或名称快速识别并自动带出资产名称、市场、类型和币种；当前价格/最新净值缺失时暂按成本价估值并标记待获取价格；期初价、汇率、现金流、费用、数据来源和复盘假设可在完整资产字段中补全；持仓列表展示数据状态和已实现收益，历史持仓展示已实现收益和复盘状态。
- 持仓价值：按用户录入价格和汇率计算成本、市值、未实现收益和收益率；资产模块内可切换持仓列表和历史持仓，支持编辑资产全部字段，也支持填写卖出数量、卖出时间、卖出价格、费用税费和原因后进行分批卖出/减仓，清仓时转入历史持仓并累计已实现收益，同时弹窗建议用户写清仓复盘。
- 分析：展示复杂收益曲线、基准对比、自定义日期筛选，并解释组合价值变化；拆分净投入/提现、价格变动、汇率变动、分红/利息、手续费、税费、手动调整和未归因差异，各项贡献加总后对齐组合价值变化。行情缓存缺失或同步失败时，提供折叠隐藏的临时手动补录价格/净值入口。
...
- 当前录入：基于用户录入的资产数量、价格、成本、费用和汇率计算，适合查看当前持仓、市值和配置。
- 估算趋势：在缺少真实历史价格或估值快照时，基于持仓成本、当前价格和日期推导中间点，只用于辅助观察，不代表真实每日净值或真实回撤。
- 行情缓存：基准和部分资产价格来自本地行情缓存，可能延迟或缺失，应以来源和时间戳为准。
- 演示估算：示例数据用于展示流程，不包含真实资产记录。

后续生产版本应优先按用户实际持仓按需补齐历史价格和汇率：当前持有资产从最早买入日到今天，历史清仓资产从买入日到清仓日，基准只同步用户选择的少量指数或 ETF。图表应优先使用真实历史价格或每日估值快照；缺失时显式提示，不静默伪装为真实历史走势。
...
2. 录入一笔现金资产，确认表单显示现金金额，市场和账户类型自动切换。
3. 录入一笔数字资产或黄金类资产，确认分类、市场和币种符合预期。
4. 在总览查看总资产、区间收益、资产类别分布、市场分布和账户列表。
5. 在分析中查看区间变化拆解，并手动补录一次价格/净值。
6. 对一笔资产做分批卖出，再做清仓，确认历史持仓和已实现收益展示正常。
7. 点击清仓复盘弹窗中的“去写复盘笔记”，确认能进入笔记写作区。
8. 导出 JSON 备份，刷新页面后粘贴导入，确认能恢复数据。
9. 在手机宽度下打开页面，确认表格可横向滚动，关键按钮可点击。
...
API 常驻运行时会默认按本机时区每天 `22:00` 执行一次价格同步，等同于抓取近 7 天行情后把最新可用价格应用到当前 API 内存资产。可用环境变量调整或关闭：
...
自动抓取近 7 天价格并应用最新价格到 API 内存资产状态：
...
- 手动点击“同步价格”会立即调用 `/api/market-data/sync-daily`，抓取近 7 天价格并把最新结果写回浏览器本地资产。
- 页面启动后会检查当天是否已经自动同步；若未同步，会按当前本地持仓代码自动触发一次同一接口，并以最新抓取到的价格为准。本地 MVP 需要先运行 API 服务：`npm run api:start`。
...
curl -X POST http://localhost:4180/api/market-data/sync-daily \
  -H "Content-Type: application/json" \
  -d '{"symbols":["000300","SPY","NDX"],"days":30}'
```
...
计价货币只影响页面展示，不修改资产原始录入数据。选择人民币时，人民币资产按原始人民币价格展示，美元资产按设置汇率折算；选择美元时，人民币资产按设置汇率折算为美元；选择 BTC 时，美元资产按 BTC/美元汇率折算，人民币资产先按美元/人民币汇率折算为美元，再折算为 BTC。金额展示使用货币符号，例如 `¥`、`$`、`₿`。
