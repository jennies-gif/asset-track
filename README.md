...
- 总览：作为专业资产 dashboard 展示总资产、累计收益、收益率、最近变化、数据状态、收益表现基准对比、资产配置概览、主要持仓、最近交易、最近复盘和待补全信息；更完整的基准选择、趋势对比和自定义日期筛选集中在分析页。
...
- 分析：展示复杂收益曲线、基准对比、自定义日期筛选，并解释组合价值变化；拆分净投入/提现、价格变动、汇率变动、分红/利息、手续费、税费、手动调整和未归因差异，各项贡献加总后对齐组合价值变化。行情缓存缺失或同步失败时，提供折叠隐藏的临时手动补录价格/净值入口；API 每日定时同步会抓取收益表现对比的默认基准缓存。
...
页面中的数据仅供记录和分析，不构成投资建议。私人资产、交易、复盘和设置默认保存在浏览器本地，不作为私人资产明细上传；证券代码、市场和日期范围等公共查询字段可能发送给行情服务。请在集中录入或重要修改后导出 JSON 备份。
...
API 常驻运行时会默认按本机时区每天 `22:00` 执行一次价格同步，默认按所有录入过且有代码资产的首次持有日期回补历史价格，并同步少量系统默认基准；不会抓取完整资产资源库。可用环境变量调整或关闭：
