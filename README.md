...
- 分析：展示复杂收益曲线、基准对比、自定义日期筛选，并解释组合价值变化；拆分净投入/提现、价格变动、汇率变动、分红/利息、手续费、税费、手动调整和未归因差异，各项贡献加总后对齐组合价值变化。行情缓存缺失或同步失败时，提供折叠隐藏的临时手动补录价格/净值入口；API 每日定时同步会抓取全部分析基准缓存。
...
页面中的数据仅供记录和分析，不构成投资建议。私人资产、交易、复盘和设置默认保存在浏览器本地，不作为私人资产明细上传；证券代码和市场会作为不关联用户的公共行情同步目标保存在服务端，历史窗口会发送给行情服务，但数量、成本、账户、交易、备注和精确持有日期不会随行情同步上传。请在集中录入或重要修改后导出 JSON 备份。
...
- 新增资产成功后会立即调用 `/api/market-data/sync-daily`，按本地首次持有日计算完整历史窗口，并确保该资产和全部分析基准覆盖至今天；已缓存区间会复用，只增量抓取未覆盖的首尾区间。请求不上传精确持有日期、数量、成本、账户或备注；返回的公共历史价格会在浏览器本地生成该资产的每日价格并刷新总资产趋势。
- 手动点击“同步价格”会调用同一接口，更新所有已录入代码和全部分析基准；已存在的历史缓存继续复用。
- 资产代码会登记为不关联用户的公共行情同步目标。API 每天自动更新这些目标和全部分析基准，因此用户重新打开页面时可直接读取最新公共缓存；页面启动后仍会进行一次当天覆盖检查。本地 MVP 需要先运行 API 服务：`npm run api:start`。

行情触发边界：完整资产资源库只用于搜索和识别，不会默认抓取价格。价格抓取只由四类事件触发：用户搜索/lookup 单个代码、已登记公共行情目标的每日定时同步、全部系统分析基准的每日定时同步、用户点击“同步价格”时同步本地资产和全部基准。
