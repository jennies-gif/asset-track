...
用途：为新增资产匹配公共标的并取得最新公共行情。查询顺序为服务端资源库精确匹配、当前支持范围内的标准化公共代码发现、公共行情验证。运行时发现的代码只有在取得有效公共价格后才会补入资源库；代码格式本身不能证明资产真实存在。自由文本名称未命中资源库时不自动转发给第三方数据源。

响应补充：

- `addedToRegistry`：本轮是否在行情验证成功后补入公共资源库；
- `status=multiple`：名称或代码只形成多个模糊候选，前端必须让用户选择，不能默认取第一项；
- `status=not_found`：当前资源库和公共数据源都未验证到对应资产，前端仍允许手动保存；
- `price`：最新公共估值价格及来源、价格日期或时点。

当前 local-first 种子版不向行情服务发送首次持有日期，也不由 lookup 服务端推断买入价格。前端默认持有日期为今天时，可以用 `price` 自动填写可编辑的买入价格；用户选择其他日期时，通过 `POST /api/market-data/sync-daily` 发送本地计算出的 `days`，再在浏览器本地从返回的历史记录中选择当天或此前最近交易日价格。买入日期和买入价格只保存在浏览器本地。除 `query` 外的查询字段返回 `400 request_field_not_allowed`。
