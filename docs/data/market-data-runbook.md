...
- 用户手动“同步价格”不创建私人资产回补任务；它通过公共行情接口检查本地资产与全部分析基准的历史覆盖，并增量补缺口和最新交易日。
...
收益表现对比基准会随无参数的 `npm run data:daily` 一起抓取；手动指定 `--symbols=` 时只抓指定代码。前端“同步价格”提交用户已录入资产并要求包含全部分析基准；API 每日固定同步会读取已登记的公共行情同步目标，并与全部分析基准一起更新。

行情抓取触发规则：

- 用户搜索或 lookup 某个代码时，只触发该代码的按需抓取。
- 用户录入过的资产代码，会登记为不关联用户的公共行情同步目标，并在 API 每日定时同步中被抓取。
- 系统收益表现对比基准，会在每日定时任务中抓取。
- 用户点击“同步价格”时，实时抓取本地已录入资产和全部分析基准。
