...
当前根目录静态种子版在新增资产后调用 `/api/market-data/sync-daily` 执行公共行情“确保覆盖”：仅发送代码、首次持有日期和今天，不发送数量、成本、账户或备注。服务端先读取按代码共享的公共行情缓存；缓存已覆盖请求区间时不重复抓取，只在首端或尾端存在缺口时增量抓取。响应中的公共历史价格回到浏览器后，按本地资产生成每日价格并刷新趋势。
